...
## Frontend, Web Collaboration

- 브라우저 렌더링 파이프라인은 어떻게 동작하나요?
- reflow와 repaint의 차이는 무엇인가요?
- SPA, CSR, SSR의 차이는 무엇인가요?
- 서버 사이드 렌더링과 클라이언트 사이드 렌더링은 백엔드 API 설계에 어떤 영향을 주나요?
- 쿠키, localStorage, sessionStorage의 차이는 무엇인가요?
- SameSite 쿠키 옵션은 왜 필요한가요?
- CORS는 왜 발생하고 서버에서는 어떻게 대응하나요?
- SOP를 우회하려는 방식에는 어떤 위험이 있나요?
- JWT를 브라우저에 저장할 때 어떤 점을 주의해야 하나요?
- XSS 공격이란 무엇이며 백엔드 API는 어떤 방어를 함께 해야 하나요?
- CSRF 공격은 프론트엔드와 백엔드에서 각각 어떻게 방어하나요?
- Cache-Control, ETag, stale-while-revalidate는 각각 어떤 상황에서 사용하나요?
- CDN을 사용할 때 API 서버와 정적 리소스 캐싱 정책은 어떻게 달라야 하나요?
- Content-Type 헤더는 왜 중요한가요?
- `<a>` 태그로 외부 페이지를 열 때 보안상 고려할 점은 무엇인가요?
- 웹 접근성이란 무엇이고 서비스 품질과 어떤 관련이 있나요?
- Core Web Vitals란 무엇인가요?
- 백엔드 API 응답이 느려 프론트엔드 사용성이 나빠질 때 어떻게 진단하나요?
- 백엔드와 프론트엔드가 API 계약을 안정적으로 관리하려면 어떻게 해야 하나요?
- 프론트엔드 E2E 테스트는 백엔드 변경과 어떤 관련이 있나요?
